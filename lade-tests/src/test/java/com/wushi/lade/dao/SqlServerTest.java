...
public class SqlServerTest {
     ...
     * 指定实现类为SqlServer
     */
    @Resource(name = "sysUserBySqlServerServiceImpl")
    private ISysUserService sysUserService;

    @Resource(name = "sysUserAccountBySqlServerServiceImpl")
    private ISysUserAccountService sysUserAccountService;

    @Resource(name = "testAutoIncrementBySqlServerServiceImpl")
    private ITestAutoIncrementBySqlServerService testAutoIncrementService;

    @Resource(name = "testBatchInsertBySqlServerServiceImpl")
    private ITestBatchInsertBySqlServerService testBatchInsertService;
     ...
     * @date 2020/1/8 15:17
     */
    private TestAutoIncrementBySqlServer insertTestAutoIncrement() {
        TestAutoIncrementBySqlServer testAutoIncrementByMysql = new TestAutoIncrementBySqlServer();
    ...
    /**
     * 批量插入
     * 【sqlserver驱动不予支持，批量插入返回主键的形式，所以Mybatis-Plus该通用方法不支持！！！】
     *
     * @return void
     * @author wushi
     * @date 2020/1/8 15:45
    ...
    private void insertBatch() {
        ...
        List<TestBatchInsertBySqlServer> testBatchInsertBySqlServerList = new ArrayList<>();
        for (int i = 0; i < 5; i++) {
            TestBatchInsertBySqlServer testBatchInsertBySqlServer = new TestBatchInsertBySqlServer();
            testBatchInsertBySqlServer.setName("name_" + i);
            testBatchInsertBySqlServer.setAddTime(LocalDateTime.now());
            testBatchInsertBySqlServerList.add(testBatchInsertBySqlServer);
        }
        // 批量插入测试
        testBatchInsertService.saveBatch(testBatchInsertBySqlServerList);
        // 自增ID结果集
        List<Integer> result = new ArrayList<>();
        // 根据TestAutoIncrement的@TableId自动映射返回testAutoIncrement变量
        for (TestBatchInsertBySqlServer testBatchInsertByMysql : testBatchInsertBySqlServerList) {
    ...
    public void update() {
        ...
        TestAutoIncrementBySqlServer testAutoIncrementBySqlServer = insertTestAutoIncrement();
        // 创建Update条件
        UpdateWrapper<TestAutoIncrementBySqlServer> updateWrapper = new UpdateWrapper<>();
        // 创建Set条件，Where id=? 条件
        updateWrapper.lambda().set(TestAutoIncrementBySqlServer::getName, "UpdateName")
                .eq(TestAutoIncrementBySqlServer::getId, testAutoIncrementBySqlServer.getId());
    ...
    public void updateBatch() {
        ...
        List<TestBatchInsertBySqlServer> testBatchInsertBySqlServerList = testBatchInsertService.list();
        // 在批量更新前先进行【插入】（如果该表无数据的情况下进行）
        if (testBatchInsertBySqlServerList == null || testBatchInsertBySqlServerList.size() == 0) {
            insertTestAutoIncrement();
        }
        // 随机数区分UpdateName
        Random random = new Random();
        // 批量修改Name字段
        testBatchInsertBySqlServerList.forEach(testBatchInsert -> testBatchInsert.setName("UpdateName" + random.nextInt(1000)));
        System.out.println("修改后的数据：" + testBatchInsertBySqlServerList);
        // 批量更新
        boolean b = testBatchInsertService.updateBatchById(testBatchInsertBySqlServerList);
        if (b) {
    ...
    public void deleteById() {
        ...
        // 新增一条自增记录
        TestAutoIncrementBySqlServer testAutoIncrementBySqlServer = insertTestAutoIncrement();
        int id = testAutoIncrementBySqlServer.getId();
        // 删除这条记录
    ...
    public void deleteByCondition() {
        ...
        TestAutoIncrementBySqlServer testAutoIncrementBySqlServer = insertTestAutoIncrement();
        // 创建查询条件（按ID）
        QueryWrapper<TestAutoIncrementBySqlServer> queryWrapper = new QueryWrapper<>();
        queryWrapper.lambda().eq(TestAutoIncrementBySqlServer::getId, testAutoIncrementBySqlServer.getId());
        // 删除记录
        boolean b = testAutoIncrementService.remove(queryWrapper);
        if (b) {
            System.out.println("成功删除记录，id:" + testAutoIncrementBySqlServer.getId());
        }
