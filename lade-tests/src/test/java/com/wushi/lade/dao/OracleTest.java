...
public class OracleTest {
     ...
     * 指定实现类为Oracle
     */
    @Resource(name = "sysUserByOracleServiceImpl")
    private ISysUserService sysUserService;

    @Resource(name = "sysUserAccountByOracleServiceImpl")
    private ISysUserAccountService sysUserAccountService;

    @Resource(name = "testAutoIncrementByOracleServiceImpl")
    private ITestAutoIncrementByOracleService testAutoIncrementService;

    @Resource(name = "testBatchInsertByOracleServiceImpl")
    private ITestBatchInsertByOracleService testBatchInsertService;
     ...
     * Insert单条记录，返回自增主键ID
     *
     * @return TestAutoIncrementByOracle
     * @author wushi
     * @date 2020/1/8 15:17
     */
    private TestAutoIncrementByOracle insertTestAutoIncrement() {
        TestAutoIncrementByOracle testAutoIncrementByOracle = new TestAutoIncrementByOracle();
        testAutoIncrementByOracle.setName("admin");
        testAutoIncrementByOracle.setAddTime(LocalDateTime.now());
        testAutoIncrementService.save(testAutoIncrementByOracle);
        // 根据TestAutoIncrement的@TableId自动映射返回testAutoIncrement变量
        System.out.println("TestAutoIncrement-insert返回自增主键：" + testAutoIncrementByOracle.getId());
        return testAutoIncrementByOracle;
    }
    ...
    public void insertBatch() {
        ...
        List<TestBatchInsertByOracle> testBatchInsertList = new ArrayList<>();
        for (int i = 0; i < 5; i++) {
            TestBatchInsertByOracle testBatchInsert = new TestBatchInsertByOracle();
            testBatchInsert.setName("name_" + i);
            testBatchInsert.setAddTime(LocalDateTime.now());
            testBatchInsertList.add(testBatchInsert);
        }
        // 批量插入测试
        testBatchInsertService.saveBatch(testBatchInsertList);
        // 自增ID结果集
        List<Integer> result = new ArrayList<>();
        // 根据TestAutoIncrement的@TableId自动映射返回testAutoIncrement变量
        for (TestBatchInsertByOracle testBatchInsert : testBatchInsertList) {
            result.add(testBatchInsert.getId());
        }
    ...
    public void update() {
        ...
        TestAutoIncrementByOracle testAutoIncrement = insertTestAutoIncrement();
        // 创建Update条件
        UpdateWrapper<TestAutoIncrementByOracle> updateWrapper = new UpdateWrapper<>();
        // 创建Set条件，Where id=? 条件
        updateWrapper.lambda().set(TestAutoIncrementByOracle::getName, "UpdateName")
                .eq(TestAutoIncrementByOracle::getId, testAutoIncrement.getId());
    ...
    public void updateBatch() {
        ...
        List<TestBatchInsertByOracle> testBatchInsertList = testBatchInsertService.list();
        // 在批量更新前先进行【批量插入】（如果该表无数据的情况下进行）
        if (testBatchInsertList == null || testBatchInsertList.size() == 0) {
            insertBatch();
        }
        // 随机数区分UpdateName
        Random random = new Random();
        // 批量修改Name字段
        testBatchInsertList.forEach(testBatchInsert -> testBatchInsert.setName("UpdateName" + random.nextInt(1000)));
        System.out.println("修改后的数据：" + testBatchInsertList);
        // 批量更新
        boolean b = testBatchInsertService.updateBatchById(testBatchInsertList);
        if (b) {
    ...
    public void deleteById() {
        ...
        // 新增一条自增记录
        TestAutoIncrementByOracle testAutoIncrement = insertTestAutoIncrement();
        int id = testAutoIncrement.getId();
        // 删除这条记录
    ...
    public void deleteByCondition() {
        ...
        TestAutoIncrementByOracle testAutoIncrement = insertTestAutoIncrement();
        // 创建查询条件（按ID）
        QueryWrapper<TestAutoIncrementByOracle> queryWrapper = new QueryWrapper<>();
        queryWrapper.lambda().eq(TestAutoIncrementByOracle::getId, testAutoIncrement.getId());
        // 删除记录
        boolean b = testAutoIncrementService.remove(queryWrapper);
        if (b) {
            System.out.println("成功删除记录，id:" + testAutoIncrement.getId());
        }
